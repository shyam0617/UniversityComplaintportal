...
app.post('/submit-grievance', (req, res) => {
  ...
  const { id, name, year, issue, description,email } = req.body;
...
app.get('/fetchTotalStatistics', async (req, res) => {
  try {
    const totalComp = await Query.countDocuments();
    const totalSolved = await Query.countDocuments({ Status: 'Completed' });
    const totalPending = await Query.countDocuments({ Status: 'Pending' });

    console.log({ totalComp, totalSolved, totalPending }); // Debugging

    res.json({ totalComp, totalSolved, totalPending });
  } catch (error) {
...
app.get('/fetchIssueStatistics', async (req, res) => {
  const { issue } = req.query;
  try {
    const totalComp = await Query.countDocuments({ Issue: issue });
    const totalSolved = await Query.countDocuments({ Issue: issue, Status: 'Completed' });
    const totalPending = await Query.countDocuments({ Issue: issue, Status: 'Pending' });
